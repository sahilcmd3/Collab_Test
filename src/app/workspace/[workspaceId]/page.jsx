  ...
  useEffect(() => {
    const fetchWorkspace = async () => {
      ...
      if (workspaceData.length > 0) {
        setWorkspaceName(workspaceData[0].name);

        const membersQuerySpec = {
        ...
        const { resources: membersCountData } = await container.items.query(membersQuerySpec).fetchAll();
        setMembersCount(membersCountData[0]);
      } else {
