...
import { msalInstance, container } from "@/config/azure";
...
const Dashboard = () => {
      ...
      try {
        const query = {
          ...
          parameters: [
            {
              name: "@userId",
              value: user.homeAccountId,
            },
          ],
        };
        const { resources: workspaceData } = await queryItems(query);
  ...
  const handleCreateWorkspace = async () => {
    ...
    try {
      ...
      const { resource: createdWorkspace } = await createItem(workspace);

      setWorkspaces([
        ...workspaces,
        { id: createdWorkspace.id, ...createdWorkspace },
      ]);
              ...
              workspaces.map((ws) => (
                ...
                >
                  <CardContent className="p-6 flex flex-col gap-4">
                    <Link href={`/workspace/${ws.id}`} className="block">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex justify-between items-center">
                          <p className="text-sm text-yellow-400 font-medium">Role: {ws.members.find((m) => m.userId === user.homeAccountId)?.role || "Unknown"}</p>
                    ...
                    {ws.members.find((m) => m.userId === user.homeAccountId)?.role === "owner" && (
                      <Button
                        variant="ghost"
                        size="icon"
