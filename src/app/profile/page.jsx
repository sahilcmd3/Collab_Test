...
import { msalInstance, container } from "@/config/azure"; // Azure config
import { useRouter } from "next/navigation";
import { createItem, readItem, updateItem, deleteItem, queryItems } from "@/lib/azureDb";
import { sendPasswordResetEmail } from "@/lib/azureAuth";
...
const Profile = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      fetchInvites(currentUser.homeAccountId);
    } else {
  ...
  const fetchInvites = async (userId) => {
    try {
      ...
      const { resources: userData } = await queryItems(query);
  ...
  const handleAcceptInvite = async (workspaceId) => {
    ...
    try {
      ...
      await updateItem(`workspaces/${workspaceId}/members`, user.homeAccountId, member);

      // Step 2: Remove the invite from the user's document
      const userUpdate = {
        invites: arrayRemove(workspaceId),
      };
      await updateItem("users", user.homeAccountId, userUpdate);
  ...
  const isGoogleUser = user && user.idTokenClaims["idp"] === "google";
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <Avatar className="w-16 h-16 mb-4 border-2 border-blue-500">
            <AvatarImage src={user?.idTokenClaims?.picture || "/robotic.png"} alt="Profile" />
            <AvatarFallback>U</AvatarFallback>
          </Avatar>
          <h1 className="text-xl font-semibold text-blue-400">{user?.name || "User"}</h1>
