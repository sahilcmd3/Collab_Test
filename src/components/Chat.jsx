...
function Chatroom({ workspaceId, setIsChatOpen }) {
  ...
  const userId = msalInstance.getAllAccounts()[0].homeAccountId;
  const name = msalInstance.getAllAccounts()[0].name;
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const querySpec = {
        query: "SELECT * FROM c WHERE c.workspaceId = @workspaceId ORDER BY c.createdAt",
        parameters: [
          {
            name: "@workspaceId",
            value: workspaceId,
      ...
      const { resources: messagesData } = await container.items.query(querySpec).fetchAll();
  ...
  const sendMessage = async () => {
    ...
    const imageUrl = msalInstance.getAllAccounts()[0].idTokenClaims.picture;
    ...
    try {
      ...
      if (aiPrompt) {
        ...
        await container.items.create(aiMessage);
      }
  ...
  const clearChat = async () => {
    try {
      ...
      const deletePromises = messagesData.map((message) => container.item(message.id).delete());
