...
const NavPanel = ({ workspaceId, openFile }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { resources: membersData } = await container.items.query(membersQuery).fetchAll();
        const member = membersData.find((m) => m.userId === user.uid);
        if (member) setUserRole(member.role);

        const foldersQuery = {
          query: "SELECT * FROM c WHERE c.workspaceId = @workspaceId AND c.type = 'folder'",
          parameters: [{ name: "@workspaceId", value: workspaceId }],
        };
        const { resources: foldersData } = await container.items.query(foldersQuery).fetchAll();
        ...
        const filesQuery = {
          query: "SELECT * FROM c WHERE c.workspaceId = @workspaceId AND c.type = 'file'",
          parameters: [{ name: "@workspaceId", value: workspaceId }],
        };
        const { resources: filesData } = await container.items.query(filesQuery).fetchAll();
        setFiles(filesData);
      } catch (error) {
  ...
  const handleDrop = async (e, targetFolderId) => {
    ...
    try {
      ...
      const itemQuery = {
        ...
        parameters: [{ name: "@id", value: draggedItem.id }],
      };
      const { resources: itemData } = await container.items.query(itemQuery).fetchAll();
      if (itemData.length > 0) {
        const item = itemData[0];
        item[fieldName] = targetFolderId || null;
  ...
  const createItem = async (folderid) => {
    ...
    try {
      ...
      if (creatingType === "folder") {
        newItem.parentFolderId = creatingParentFolderId;
      } else {
        newItem.folderId = creatingParentFolderId;
      }
  ...
  const renameItem = async () => {
    ...
    try {
      const itemQuery = {
        query: "SELECT * FROM c WHERE c.id = @id",
        parameters: [{ name: "@id", value: renamingItem.id }],
      };
      const { resources: itemData } = await container.items.query(itemQuery).fetchAll();
      if (itemData.length > 0) {
        const item = itemData[0];
        item.name = renamingItem.name;
        await container.item(item.id).replace(item);
      }
  ...
  const deleteItem = async (type, id) => {
    try {
      ...
      const { resources: itemData } = await container.items.query(itemQuery).fetchAll();
      if (itemData.length > 0) {
        await container.item(itemData[0].id).delete();
      }
      if (type === "folder") {
        const nestedFolders = folders.filter((folder) => folder.parentFolderId === id);
        for (const nestedFolder of nestedFolders) {
          await deleteItem("folder", nestedFolder.id);
        }
