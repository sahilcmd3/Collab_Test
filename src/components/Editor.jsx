...
export default function CodeEditor({ file }) {
  ...
  useEffect(() => {
    ...
    const fetchFileContent = async () => {
      ...
      try {
        const querySpec = {
          query: "SELECT * FROM c WHERE c.id = @fileId AND c.workspaceId = @workspaceId",
          parameters: [
            { name: "@fileId", value: file.id },
            { name: "@workspaceId", value: file.workspaceId },
          ],
        };

        const { resources: fileData } = await container.items.query(querySpec).fetchAll();
        if (fileData.length > 0) {
          setUpdatedCode(fileData[0].content || "");
        }
      } catch (error) {
  ...
  const autoSaveFile = async (content) => {
    ...
    try {
      ...
      const { resources: fileData } = await container.items.query(querySpec).fetchAll();
      if (fileData.length > 0) {
        const fileItem = fileData[0];
        fileItem.content = content;
        await container.item(fileItem.id).replace(fileItem);
      }
    } catch (error) {
